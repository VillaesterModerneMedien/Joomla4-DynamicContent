...
class ArticleEventType
{
    ...
    public static function resolve($article, $args, $context, $info)
    {
        ...
        Factory::getApplication()->registerEvent('onBeforeRender', function () use ($article, $key, $marker) {
    ...
    protected static function applyContentPlugins($article)
    {
        ...
        $results = $app->triggerEvent('onContentAfterTitle', ['com_content.article', &$article, &$article->params]);
        $article->event->afterDisplayTitle = trim(implode("\n", $results));

        $results = $app->triggerEvent('onContentBeforeDisplay', ['com_content.article', &$article, &$article->params]);
        $article->event->beforeDisplayContent = trim(implode("\n", $results));

        $results = $app->triggerEvent('onContentAfterDisplay', ['com_content.article', &$article, &$article->params]);
